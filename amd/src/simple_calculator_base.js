...
/**
 * Adds the base function of a simple base_calculator.
 *
...
define(
  ...
  function($, cstr, ShuntingYardConverter, RPNEvaluator) {
    var base_calculator = {
        ...
        $(SELECTORS.OPERATIONS).each(function() {
          $(this).on("click", function() {
            ...
            if (
              ...
              memory.previousOperand =
                memory.previousOperand.toString() +
                memory.operation.toString() +
              ...
              memory.operation === null &&
              base_calculator.hasNumbers(memory.currentOperand) &&
              memory.previousOperand.includes("=")
            ) {
      ...
      deleteButton: function() {
        $(SELECTORS.DELETE).on("click", function() {
          if (memory.currentOperand !== "") {
            ...
            if (
              memory.currentOperand.toString()[
                memory.currentOperand.length - 1
              ...
              if (
                firstParenthesis == memory.currentOperand.length - 1 &&
                !memory.currentOperand.toString()[firstParenthesis - 1].match("/[0-9]/")
              ) {
                // It has an Negate so remove the neg(.
                memory.currentOperand = memory.currentOperand
                  .toString()
                  .replace("neg(", "");
                // Remove the last Character ( The closing parenthesis).
                memory.currentOperand = memory.currentOperand
            ...
            // Get nearest Operand.
            var nearestOperand = base_calculator.getNearestOperation(
              memory.previousOperand.toString(),
              true
            );
      ...
      popoutButton: function() {
        $(document).on("click", SELECTORS.POPOUT, () => {
          if (!draggable) {
            ...
            cstr
              .get_string("calculator_close", "block_simple_calculator")
              .done(function(popup) {
            ...
            cstr
              .get_string("calculator_popout", "block_simple_calculator")
              .done(function(popup) {
                $(SELECTORS.POPOUT_TEXT).text(popup);
              });
          }
      ...
      /**
         * isOperator
         * Matches and returns boolean if the key is an Operator
         * @param {string} key
         * @return {boolean}
         *
         */
      isOperator: function(key) {
        return key.match(OPERATOR_PARSE_REGEX);
      },
      ...
      getNearestOperation: function(operationString, removeFromString = false) {
        ...
        for (var x = operationString.length; x >= 0; x--) {
          ...
          if (
            operationString[x] !== undefined &&
            operationString[x].match(OPERATOR_REGEX)
          ) {
        ...
        if (removeFromString) {
          ...
          if (countOperations !== 0) {
            ...
            operationStringSliced = operationString.slice(
              0,
              (countOperations >= 1 ? endIndex - 1 : endIndex) -
      ...
      /**
         * hasNumbers
         * Verify that the operand has a numbers.
         * @param {string} str
         */
      ...
      calculate: function() {
        ...
        var validateParenthesis = this.validateCorrectParenthesis(
          memory.previousOperand.toString() +
        ...
        var result = memory.previousOperand.slice(
          0,
          memory.previousOperand.length - 1
        );
        ...
        if (validateParenthesis) {
          ...
          result = new RPNEvaluator(
            new ShuntingYardConverter().toRPN(result)
          ).eval();
      ...
      update: function(
        ...
        currentOperand =
          currentOperand === null ? memory.currentOperand : currentOperand;
        previousOperand =
          previousOperand === null ? memory.previousOperand : previousOperand;
